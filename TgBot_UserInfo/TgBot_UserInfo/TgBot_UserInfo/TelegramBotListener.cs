...
{
    private static string BotToken = "8072598213:AAGUeTJONeXxoi9_h6A1R99gluGz97p-ncU";

    private long AdminID = 7128017505;
    ...
    public async Task HandleUpdateAsync(ITelegramBotClient bot, Update update, CancellationToken cancellationToken)
    {
        if (update.Type == UpdateType.Message)
        {
            ...
            if (message.Text == "Malumot Kiritish")
            {

                if (userObject.FirstName is null)
                {
                    await bot.SendTextMessageAsync(user.Id, "Avval Isminggizni Kiriting : ", cancellationToken: cancellationToken);
                ...
                else if(userObject.PhoneNumberr is null)
                {
                    ...
                    await bot.SendTextMessageAsync(user.Id, "Telefon raqamni kiriting (90 090 90 90):", cancellationToken: cancellationToken);
                }
                else
                {
    ...
    $"TelegramID : {userObject.TelegramUserId}";
            ...
            if (message.Text == "Malumot Ko'rish")
            {
                if (userObject.FirstName is null)
                {
        ...
        $"TelegramID : {userObject.TelegramUserId}";
            ...
            if (message.Text == "Malumot O'Chirish")
            {
                ...
                var keyboard = new ReplyKeyboardMarkup(new[]
            {
                    new[]
                    {
                        new KeyboardButton("Malumot Kiritish"),
                        new KeyboardButton("Malumot Ko'rish"),
                    },
                    new[]
                    {
                        new KeyboardButton("Malumot O'Chirish"),
                    },
                })
                { ResizeKeyboard = true };

                await bot.SendTextMessageAsync(user.Id, "Botimizga xush kelibsiz\n o'zingizga kerakli bo'limni tanlang:  ", replyMarkup: keyboard);
                return;
            }
        }
